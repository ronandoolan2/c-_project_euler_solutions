...
public:
   ...
   void make_connection(node *leaf);
   int make_connections(node *leaf);
   node *get_root();
   node *root;
   void levelhelper(int x, int y, node* t);
   void make_connections_helper(node *leaf, int x, int y);
...
void tree::make_connection(node *leaf)
{
   cout << leaf->right->data << endl;
   leaf->left->right = leaf->right->left;
}
int tree::make_connections(node *leaf)
{
/*   if(leaf->left->left != NULL) 
   {
      cout << "connecting" << leaf->data << endl;
      if(leaf->right->right == NULL)
         cout << "right does not exist" << endl;
      make_connection(leaf);
      if(leaf->left->left->left != NULL)
      {
         make_connection(leaf->left);
         make_connection(leaf->right);
      }
      //return;
   }
   else
   ...
   cout << "make more" << leaf->data << endl;
   make_connections(leaf->left);
   make_connections(leaf->right);
  */
   for(int j; j< 13; j++)
   {
      make_connections_helper(leaf, j, 0);
   } 
}
void tree::make_connections_helper(node* t, int x, int y)
{
    if(t == NULL)
        return;
    else if(x == y)
    ...
    {
        y++;
        make_connections_helper(t->left, x, y);
        make_connections_helper(t->right, x, y);
    }
}

void tree::levelhelper(int x, int y, node* t)
{
    if(t == NULL)
        return;
    else if(x == y)
...
std::ifstream myfile("triangle4.txt");
int main()
{
 ...
 //  node *one;
  ...
  if (myfile.is_open())
  {
   ...
   three4->data = 3;
   two3->right = three4;*/
   triangle->make_connections(triangle->get_root());
   //triangle->make_connection(triangle->get_root()->left);
   //triangle->make_connection(triangle->get_root()->right);
   for(int k = 0; k < 14; k++)
   {
      triangle->printlevel(k);
      cout << endl;
   }
   cout << "Max path sum is " << maxSumPath(triangle->get_root());   
